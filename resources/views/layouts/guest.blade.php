...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'ParadiseDollz') }}</title>

        @vite(['resources/css/app.css', 'resources/js/app.js'])
    </head>
    <body class="font-sans antialiased pd-dark-surface">
        <div class="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden px-4 py-10">
            ...
            {{-- Explicit max-width + shrink-0 so the card never expands to full viewport if utilities fail to load --}}
            <div
                class="relative z-10 mx-auto w-full shrink-0 max-w-md"
